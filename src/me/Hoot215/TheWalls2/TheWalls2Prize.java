...
public class TheWalls2Prize {
	...
	public static void giveItemPrize(TheWalls2 plugin, Player player) {
		String[] prize = plugin.getConfig()
				.getString("general.prize-item").split(":");
		...
		player.sendMessage(ChatColor.AQUA + "[TheWalls2] "
				+ ChatColor.GREEN + "Here's your prize!");
	}
	...
	public static void giveMoneyPrize(TheWalls2 plugin, Player player) {
		...
		player.sendMessage(ChatColor.AQUA + "[TheWalls2] "
				+ ChatColor.GREEN + "Here's your prize!");
		player.sendMessage(ChatColor.AQUA + "[TheWalls2] "
				+ ChatColor.GOLD + TheWalls2.economy.format(amount)
				+ ChatColor.GREEN + " has been deposited into your account");
	}
}